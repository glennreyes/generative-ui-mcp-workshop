// Workshop adaptation: Node/tsx, loopback binds, Host/Origin checks and fresh local assets.
/**
...
hostApp.use(localhostHostValidation(), localhostOriginValidation());
hostApp.use((_req, res, next) => { res.setHeader("Cache-Control", "no-store"); next(); });
...
sandboxApp.use(localhostHostValidation(), localhostOriginValidation());
...
// ============ Start both servers ============
hostApp.listen(HOST_PORT, "127.0.0.1", (err) => {
  if (err) {
...
sandboxApp.listen(SANDBOX_PORT, "127.0.0.1", (err) => {
  if (err) {
    console.error("Error starting server:", err);
    process.exit(1);
  }
