  ...
  if (event.source === window.parent) {
    ...
    if (event.data && event.data.method === RESOURCE_READY_NOTIFICATION) {
      ...
      if (typeof html === "string") {
        // Workshop adaptation: srcdoc gives the React document a normal iframe lifecycle.
        // The response CSP is inherited and the outer sandbox stays on its own origin.
        inner.srcdoc = html;
      }
    } else {
