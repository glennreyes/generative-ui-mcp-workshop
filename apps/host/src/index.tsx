// Workshop adaptation: workshop links and an accessible iframe title.
...
function Host({ serversPromise }: HostProps) {
  ...
  return (
    <>
      <header style={{ padding: "20px 24px", borderBottom: "1px solid var(--color-border, #ddd)" }}>
        <h1 style={{ fontSize: "20px", margin: "0 0 8px" }}>Generative UI workshop · local test host</h1>
        <p style={{ margin: 0 }}>Call real MCP tools and inspect their React UI. This host does not run a conversational model.</p>
        <p><a href="https://github.com/glennreyes/generative-ui-mcp-workshop">Exercises</a> · <a href="https://discord.gg/8p3uGHNMu">Build with Glenn on Discord</a> · <a href="https://reactalicante.es">React Alicante</a></p>
      </header>
...
function AppIFramePanel({ toolCallInfo, isDestroying, onTeardownComplete }: AppIFramePanelProps) {
  ...
  return (
    <div className={panelClassName}>
      <iframe ref={iframeRef} title="MCP App sandbox" />
      {messages.length > 0 && (
        <CollapsiblePanel
          icon="💬"
          label="Messages"
