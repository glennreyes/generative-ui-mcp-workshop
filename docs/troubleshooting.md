...
| `npm ci` fails                         | Use npm 11, run from the repository root, and keep the committed lockfile. Check connectivity before removing anything.                 |
| Port already in use                    | Stop another workshop copy with Ctrl+C. `npm run doctor` checks 3001, 8080 and 8081.                                                    |
| Host cannot connect                    | Keep `npm run dev` running. Open `http://localhost:3001/health`, then reload the host.                                                  |
| Tool works, no UI                      | At checkpoints 00 and 01 this is expected. Otherwise inspect tool `_meta.ui.resourceUri`, resource MIME type and `npm run build`.       |
...
| Host edits not reflected               | `npm run build -w @workshop/host`, then reload. The host is supplied infrastructure and is not watched by default.                      |
...
| `npm test` fails in a starter          | Tests describe final behavior. Follow the relevant exercise or open a solution checkpoint.                                              |
...
```sh
npm run checkpoint -- 02-react-ui ../agenda-recovery
cd ../agenda-recovery
npm ci
npm run dev
```
...
OS, `node --version`, `npm --version`, checkpoint name, exact command and the first useful error. If a tool succeeds but rendering fails, include the browser console error and whether `npm run smoke` passes.
